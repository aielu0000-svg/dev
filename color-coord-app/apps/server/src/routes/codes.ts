...
router.get('/similar/:id', (req, res) => {
  ...
  const code = db.prepare(`
    ...
    FROM codes
    WHERE id = ? AND status = 'active'
  `).get(id) as { id: string; palette: string; season_tags: string | null } | undefined;
  ...
  if (seasonTags.length > 0) {
    ...
    similarCodes = db.prepare(`
      ...
      ORDER BY c.likes DESC
      LIMIT ?
    `).all(id, ...seasonTags.map((tag: string) => `%"${tag}"%`), limit) as Code[];
  } else {
    // 季節タグがない場合は人気順
    similarCodes = db.prepare(`
      SELECT c.id, c.illustration_url, c.palette, c.likes
...
router.get('/filter', (req, res) => {
  ...
  const codes = db.prepare(query).all(...params) as Code[];
