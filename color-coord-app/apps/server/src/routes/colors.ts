...
router.get('/search', (req, res) => {
  const hexParam = req.query.hex as string;
...
router.get('/explain', (req, res) => {
  const baseHex = req.query.base as string;
  const matchHex = req.query.match as string;
...
router.post('/simulate-replace', (req, res) => {
  try {
    ...
    `).get(codeId) as { id: string; palette: string } | undefined;
    ...
    // 色を置き換え
    const newPalette = palette.map((color: any) => {
