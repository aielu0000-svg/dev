...
export class ColorExplanationService {
  private readonly harmonyExplanations: Record<HarmonyType, string> = {
   ...
   * 2色の組み合わせについて説明を生成
   */
  explainColorPair(baseHex: string, matchHex: string): ColorExplanation {
   ...
   * 配色のハーモニータイプを検出
   */
  private detectHarmonyType(
    baseHsv: { h: number; s: number; v: number },
    matchHsv: { h: number; s: number; v: number }
  ): HarmonyType {
