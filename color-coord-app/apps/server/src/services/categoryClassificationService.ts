import { ColorInfo } from './imageProcessingService.js';
...
export class CategoryClassificationService {
   ...
   * 色情報からカテゴリを推定
   */
  classifyCategories(palette: ColorInfo[]): ClassificationResult {
   ...
   * RGBから色相を計算
   */
  private getHue(rgb: { r: number; g: number; b: number }): number {
    const r = rgb.r / 255;
    const g = rgb.g / 255;
    const b = rgb.b / 255;
   ...
   * パレット全体の平均色相
   */
  private getAverageHue(palette: ColorInfo[]): number {
   ...
   * パレット全体の平均彩度
   */
  private getAverageSaturation(palette: ColorInfo[]): number {
   ...
   * パレット全体の平均明度
   */
  private getAverageBrightness(palette: ColorInfo[]): number {
