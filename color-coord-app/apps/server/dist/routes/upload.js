 ...
 * 非同期で画像を処理
 */
async function processImageAsync(historyId, imageBuffer, style, originalFilename) {
    try {
        // 進捗更新用コールバック
    ...
    `).run(codeId, `/processed/${result.id}_illustration.png`, `/uploads/${result.id}_original.jpg`, JSON.stringify(result.palette), result.style, JSON.stringify(classification.seasonTags), result.processingTime, 0, 'user_upload', 'active');
        // カテゴリを関連付け
