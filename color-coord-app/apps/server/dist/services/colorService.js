...
function hsvDistance(hex1, hex2) {
    ...
    const distance = Math.sqrt(Math.pow(hueDiff / 360, 2) * 0.5 +
        Math.pow((hsv1.s - hsv2.s) / 100, 2) * 0.3 +
        Math.pow((hsv1.v - hsv2.v) / 100, 2) * 0.2);
    return distance;
}
// Find similar colors from the database
export function findSimilarColors(targetHex, limit = 8) {
...
function extractMatchingColorsFromPopularCodes(targetHex, limit = 10) {
    const normalizedTarget = targetHex.toUpperCase();
    const matchingColors = new Map();
    // いいね数100以上の人気コーデを取得
...
export function findMatchingColors(targetHex, limit = 10) {
...
export function findCodesByColor(targetHex, threshold = 0.15) {
    const codes = db.prepare(`
    SELECT id, palette, likes FROM codes
    WHERE status = 'active'
    ORDER BY likes DESC
  `).all();
    const matchingCodeIds = [];
    for (const code of codes) {
        const palette = JSON.parse(code.palette);
        for (const item of palette) {
